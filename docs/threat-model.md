...
| 10 | **Information Disclosure** | Error message leakage | `account-service.ts:161, 179`, `require-auth.ts:107-121` | Generic error messages; no stack traces in JSON responses; `AuthenticationError` uses sanitized `code` field | None in current error paths |
