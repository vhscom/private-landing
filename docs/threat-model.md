...
| 24 | **Information Disclosure** | Unbounded `security_event` retention | `002_observability.sql` | **Gap**: No pruning trigger or retention policy — the `security_event` table grows indefinitely | Growing exposure surface of IP addresses, user agents, and event details; potential query performance degradation under sustained event volume |
